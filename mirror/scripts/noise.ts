...
export function get_smooth_noise (x : number, y : number, radius : number, width : number,  height : number) : number {
...
function get_variable_noise (x : number, y : number, grid : vector[][], active_area:{top:number, left : number, length: number}) : number {
    ...
    if (!grid[x0] || !grid[x0][y0]) throw Error(`Perlin Noise cell is out of range! Tried to access ${x0} then ${y0} of ${grid}`);
...
function generate_gradient_grid (node:number) : vector[][] {
