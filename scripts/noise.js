...
export function get_noise(x, y, radius, width, height) {
    ...
    if (!random_gradient_vector_grid[x0] || !random_gradient_vector_grid[x0][y0])
        throw Error(`Perlin Noise cell is out of range! Tried to access ${x0} then ${y0} of ${random_gradient_vector_grid}`);
    const bottom_left = {
    ...
    bottom_left["dot_product"] = Utility.dot_product(bottom_left.gradient.x, bottom_left.gradient.y, bottom_left.distance.x, bottom_left.distance.y);
    bottom_right["dot_product"] = Utility.dot_product(bottom_right.gradient.x, bottom_right.gradient.y, bottom_right.distance.x, bottom_right.distance.y);
    top_left["dot_product"] = Utility.dot_product(top_left.gradient.x, top_left.gradient.y, top_left.distance.x, top_left.distance.y);
    top_right["dot_product"] = Utility.dot_product(top_right.gradient.x, top_right.gradient.y, top_right.distance.x, top_right.distance.y);
